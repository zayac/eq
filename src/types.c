...
{
  struct type_table_key *key = (struct type_table_key*) 
      malloc (sizeof (struct type_table_key));
  struct type_hash_table *el = (struct type_hash_table*)
      malloc (sizeof (struct type_hash_table));
...
/* This function is used to set simple types in an easy way.
   If code belongs to tcl_type and size is not equal zero, we try to find the 
   relevant entries in type_name_list and type_size_list. If there are no such
   entries, we create them. Then we look for an entry in type hash table. If
   there is no such an entry, we create it.
   If code is set to EMPTY_MARK, we try to find it in a list and in case of
   unluck add a new entry to the size list.
   If size is set to zero, we try to find it in a list and in case of 
   unluck add a new entry to the name list.  */
struct type_hash_table*
...
struct type_hash_table* types_find_in_table (tree name, tree size)
{
  struct type_table_key key;
