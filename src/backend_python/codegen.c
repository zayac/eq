...
codegen_function (FILE* f, tree func)
{
  ...
  fprintf (f, "):\n");
  level++;
  error = codegen_stmt_list (f, TREE_OPERAND (func, 4));  
  level--;
  indent (f, level);
  return error;
}
...
codegen_stmt (FILE* f, tree stmt)
{
  ...
  switch (TREE_CODE (stmt))
	...
	fprintf (f, ":\n");
	level++;
	error += codegen_stmt_list (f, TREE_OPERAND (stmt, 1));
	level--;
	indent (f, level);
