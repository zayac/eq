...
typecheck_constant (tree cst)
{
  ...
  assert (TREE_CODE (cst) == ASSIGN_EXPR || TREE_CODE (cst) == IDENTIFIER, 0);
...
typecheck_stmt (tree stmt, tree ext_vars, tree vars)
{
  ...
  switch (TREE_CODE (stmt))
	...
	if (strcmp (name, "generate") == 0)
	  {
	    ret =
	      typecheck_expression (TREE_OPERAND (stmt, 1), ext_vars, vars);
  ...
  switch (TREE_CODE (expr))
	...
	if ((TREE_TYPE (lhs) == string_type_node
	    ...
	    + try_implicit_conversion (list_type_node, rhs, &rhs) == 0)
	  {
	    ...
	    TAILQ_FOREACH (tel, &TREE_LIST_QUEUE (TREE_LIST_CST (rhs)),
			   entries)
	    {
	      tree el = tel->element;
